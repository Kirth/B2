...
where
    F: Fn(&mut Executor, Vec<Value>, crate::parser::ast::Span) -> Result<Value, RuntimeError>
        + Send
        + Sync
        + 'static,
{
...
fn expect_number(value: Option<&Value>, name: &str) -> Result<f64, String> {
    match value {
        ...
        Some(v) => v
            .as_string()
            .parse::<f64>()
            .map_err(|_| format!("Expected numeric {name}")),
...
fn net_http(
    ...
    let req_map = match args.get(0) {
        Some(Value::Dict(map)) => map
            .lock()
            .map_err(|_| err(exec, span, "Request map lock poisoned"))?
            .clone(),
    ...
    let timeout =
        optional_timeout_ms(dict_get(&req_map, "timeout_ms")).map_err(|e| err(exec, span, &e))?;
    ...
    if let Some(Value::Dict(headers)) = dict_get(&req_map, "headers") {
        let hdrs = headers
            .lock()
            .map_err(|_| err(exec, span, "Headers map lock poisoned"))?;
    ...
    let response = request
        .send()
        .map_err(|e| err(exec, span, &format!("HTTP request failed: {e}")))?;
    ...
    out.insert(
        "ok".to_string(),
        Value::Bool((200.0..300.0).contains(&status)),
    );
...
fn net_tcp_listen(
    ...
    let listener = TcpListener::bind(&addr)
        .map_err(|e| err(exec, span, &format!("tcp_listen failed: {e}")))?;
    listener.set_nonblocking(true).map_err(|e| {
        err(
            exec,
            span,
            &format!("Failed to set nonblocking listener: {e}"),
        )
    })?;
...
fn make_connection_value(stream: TcpStream, default_timeout: Duration) -> Value {
...

    {
        ...
        map.insert(
            "recv".to_string(),
            native_exec(move |exec, args, span| {
                let max = expect_number(args.first(), "max_bytes")
                    .map_err(|e| err(exec, span, &e))? as i64;
                if max <= 0 {
